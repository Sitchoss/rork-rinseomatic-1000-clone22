...
class UnifiedSessionViewModel {
    ...
    func startBatch() {
        ...
        isRunning = true
        batchStartTime = Date()
        ScreenshotDedupService.shared.resetAll()
        ...
        batchTask = Task {
            ...
            while sessionIndex < self.sessions.count && !self.isStopping && !Task.isCancelled {
                ...
                while activeTasks.count >= currentLiveConcurrency {
                ...
                let task = Task {
                    ...
                    if result.session.isTerminal {
                        NotificationCenter.default.post(
                            name: .unifiedSessionCompleted,
                            object: nil,
                            userInfo: ["sessionId": result.session.id]
                        )
                    }
    ...
    func overrideSiteResult(sessionId: String, site: String, result: SiteResult) {
        guard let idx = sessions.firstIndex(where: { $0.id == sessionId }) else { return }
        if site.lowercased().contains("joe") {
            sessions[idx].joeSiteResult = result
        } else {
            sessions[idx].ignitionSiteResult = result
        }

        let joe = sessions[idx].joeSiteResult
        let ign = sessions[idx].ignitionSiteResult
        if joe.isTerminal && ign.isTerminal {
            if joe == .success || ign == .success {
                sessions[idx].classification = .validAccount
                sessions[idx].globalState = .success
            } else if joe == .permDisabled || ign == .permDisabled {
                sessions[idx].classification = .permanentBan
                sessions[idx].globalState = .abortPerm
            } else if joe == .tempDisabled || ign == .tempDisabled {
                sessions[idx].classification = .temporaryLock
                sessions[idx].globalState = .abortTemp
            } else {
                sessions[idx].classification = .noAccount
                sessions[idx].globalState = .exhausted
            }
        }
        persistSessions()
        log("Override: \(site) → \(result.shortLabel) for \(sessions[idx].credential.email)", level: .warning)
    }
