...
class LoginURLRotationService {
    ...
    static let mirrorJoeURLStrings: [String] = [
        ...
        "https://www.joefortune.eu/login",
        "https://www.joefortune.club/login",
        "https://www.joefortune.lv/login",
        "https://www.joefortune.ooo/login",
        "https://www.joefortuneonlinepokies.eu/login",
        "https://www.joefortuneonlinepokies.net/login",
        "https://www.joefortunepokies.win/login",
    ]

    static let mirrorIgnitionURLStrings: [String] = [
        // Previously base domains — now mirrors
        "https://www.ignitioncasino.lat/?overlay=login",
        "https://www.ignitioncasino.cool/?overlay=login",
        "https://www.ignitioncasino.fun/?overlay=login",
        "https://www.ignitioncasino.lv/?overlay=login",
        "https://www.ignitioncasino.eu/?overlay=login",
        // Nord-specific mirrors
        "https://www.ignitionpoker.eu/poker/tournaments?overlay=login",
        "https://www.ignitioncasino.buzz/poker/tournaments?overlay=login",
    ]

    // Base domains of all mirror URLs — used for domain-normalised membership checks so that
    // `validateAndUpdateJoeURLs()` rewriting between www/non-www variants doesn't break
    // mirror identification or removal.
    private static let allMirrorBaseDomains: Set<String> = {
        let allMirrors = mirrorJoeURLStrings + mirrorIgnitionURLStrings
        return Set(allMirrors.compactMap { urlStr -> String? in
    ...
    init() {
        joeURLs = Self.defaultJoeURLStrings.map { RotatingURL(urlString: URLNormalizer.normalize($0), isEnabled: true, lastFailure: nil, failCount: 0) }
        joeURLs += Self.disabledByDefaultJoeURLStrings.map { RotatingURL(urlString: URLNormalizer.normalize($0), isEnabled: false, lastFailure: nil, failCount: 0) }
        ignitionURLs = Self.defaultIgnitionURLStrings.map { RotatingURL(urlString: URLNormalizer.normalize($0), isEnabled: true, lastFailure: nil, failCount: 0) }
        ignitionURLs += Self.disabledByDefaultIgnitionURLStrings.map { RotatingURL(urlString: URLNormalizer.normalize($0), isEnabled: false, lastFailure: nil, failCount: 0) }
    ...
    static let disabledByDefaultJoeURLStrings: [String] = [
        "https://www.joefortune.eu/login",
        "https://www.joefortune.club/login",
        "https://www.joefortune.lv/login",
        "https://www.joefortune.ooo/login",
    ...
    static let disabledByDefaultIgnitionURLStrings: [String] = [
        "https://www.ignitioncasino.lat/?overlay=login",
        "https://www.ignitioncasino.cool/?overlay=login",
        "https://www.ignitioncasino.fun/?overlay=login",
        "https://www.ignitioncasino.lv/?overlay=login",
    ...
    func isMirrorURL(_ urlString: String) -> Bool {
        Self.allMirrorBaseDomains.contains(extractBaseDomain(from: urlString))
    }
    ...
    private func removeMirrorURLs() {
        joeURLs.removeAll { Self.allMirrorBaseDomains.contains(extractBaseDomain(from: $0.urlString)) }
        ignitionURLs.removeAll { Self.allMirrorBaseDomains.contains(extractBaseDomain(from: $0.urlString)) }
    }
    ...
    func addURL(_ urlString: String, forIgnition: Bool) -> Bool {
        let trimmed = URLNormalizer.normalize(urlString.trimmingCharacters(in: .whitespacesAndNewlines))
    ...
    func bulkImportURLs(_ text: String, forIgnition: Bool) -> (added: Int, duplicates: Int, invalid: Int) {
        ...
        for line in lines {
            ...
            urlStr = URLNormalizer.normalize(urlStr)
    ...
    func resetToDefaults(forIgnition: Bool) {
        if forIgnition {
            ...
            ignitionURLs += Self.disabledByDefaultIgnitionURLStrings.map { RotatingURL(urlString: $0, isEnabled: false, lastFailure: nil, failCount: 0) }
        } else {
            joeURLs = Self.defaultJoeURLStrings.map { RotatingURL(urlString: $0, isEnabled: true, lastFailure: nil, failCount: 0) }
            joeURLs += Self.disabledByDefaultJoeURLStrings.map { RotatingURL(urlString: $0, isEnabled: false, lastFailure: nil, failCount: 0) }
        }
        persistState()
    }
    ...
    private func injectDisabledDefaultURLs() {
        let existingJoe = Set(joeURLs.map(\.urlString))
        for urlStr in Self.disabledByDefaultJoeURLStrings where !existingJoe.contains(urlStr) {
            joeURLs.append(RotatingURL(urlString: urlStr, isEnabled: false, lastFailure: nil, failCount: 0))
        }
        let existingIgn = Set(ignitionURLs.map(\.urlString))
        for urlStr in Self.disabledByDefaultIgnitionURLStrings where !existingIgn.contains(urlStr) {
            ignitionURLs.append(RotatingURL(urlString: urlStr, isEnabled: false, lastFailure: nil, failCount: 0))
        }
    }
