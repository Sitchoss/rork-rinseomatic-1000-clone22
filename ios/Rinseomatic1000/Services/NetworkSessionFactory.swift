...
class NetworkSessionFactory {
    ...
    nonisolated private func quickSOCKS5Handshake(host: String, port: UInt16) async -> Bool {
        await withCheckedContinuation { continuation in
            ...
            nonisolated(unsafe) let timeoutWorkRef = timeoutWork
    ...
    func createDataSession(config: ActiveNetworkConfig, target: ProxyRotationService.ProxyTarget, credentialId: String? = nil) -> URLSession? {
        let sessionConfig = buildURLSessionConfiguration(for: config, target: target, credentialId: credentialId)
