...
class FlowPersistenceService {
    ...
    func loadFlows() -> [RecordedFlow] {
        ...
        do {
            ...
            let normalized = flows.map { flow -> RecordedFlow in
                let newURL = URLNormalizer.normalize(flow.url)
                if newURL == flow.url { return flow }
                return RecordedFlow(
                    id: flow.id,
                    name: flow.name,
                    url: newURL,
            ...
            logger.log("FlowPersistence: loaded \(normalized.count) flows (\(data.count) bytes)", category: .persistence, level: .info)
            return normalized
        } catch {
