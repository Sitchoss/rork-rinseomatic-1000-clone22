...
class LoginAutomationEngine {
    ...
    var maxConcurrencyOverride: Int? = nil
    var effectiveMaxConcurrency: Int { maxConcurrencyOverride ?? automationSettings.maxConcurrency }
    ...
    init(
        ...
        visionML: VisionMLService? = nil,
        debugButtonService: DebugLoginButtonService? = nil,
        networkFactory: NetworkSessionFactory? = nil,
        deadSessionDetector: DeadSessionDetector? = nil,
        replayLogger: SessionReplayLogger? = nil,
        circuitBreaker: HostCircuitBreakerService? = nil,
        challengeClassifier: ChallengePageClassifier? = nil,
        crashRecovery: WebViewCrashRecoveryService? = nil,
        lifetimeBudget: WebViewLifetimeBudgetService? = nil,
        screenshotDedup: ScreenshotDedupService? = nil,
        hostFingerprint: HostFingerprintLearningService? = nil,
        adaptiveRetry: AdaptiveRetryService? = nil,
        urlQualityScoring: URLQualityScoringService? = nil,
        confidenceEngine: ConfidenceResultEngine? = nil,
        aiProxyStrategy: AIProxyStrategyService? = nil,
        aiChallengeSolver: AIChallengePageSolverService? = nil,
        aiURLOptimizer: AILoginURLOptimizerService? = nil,
        aiFingerprintTuning: AIFingerprintTuningService? = nil,
        aiSessionHealth: AISessionHealthMonitorService? = nil,
        aiCredentialPriority: AICredentialPriorityScoringService? = nil,
        aiAntiDetection: AIAntiDetectionAdaptiveService? = nil,
        customTools: AICustomToolsCoordinator? = nil,
        aiInteractionGraph: AIReinforcementInteractionGraph? = nil,
        activityMonitor: SessionActivityMonitor? = nil,
        liveSpeed: LiveSpeedAdaptationService? = nil,
        strictDetection: StrictLoginDetectionEngine? = nil,
        crimsonOrchestrator: CrimsonSweepOrchestrator? = nil,
        janitor: WebViewLifecycleManager? = nil
    ) {
        self.logger = logger ?? .shared
        self.visionML = visionML ?? .shared
        self.debugButtonService = debugButtonService ?? .shared
        self.networkFactory = networkFactory ?? .shared
        self.deadSessionDetector = deadSessionDetector ?? .shared
        self.replayLogger = replayLogger ?? .shared
        self.circuitBreaker = circuitBreaker ?? .shared
        self.challengeClassifier = challengeClassifier ?? .shared
        self.crashRecovery = crashRecovery ?? .shared
        self.lifetimeBudget = lifetimeBudget ?? .shared
        self.screenshotDedup = screenshotDedup ?? .shared
        self.hostFingerprint = hostFingerprint ?? .shared
        self.adaptiveRetry = adaptiveRetry ?? .shared
        self.urlQualityScoring = urlQualityScoring ?? .shared
        self.confidenceEngine = confidenceEngine ?? .shared
        self.aiProxyStrategy = aiProxyStrategy ?? .shared
        self.aiChallengeSolver = aiChallengeSolver ?? .shared
        self.aiURLOptimizer = aiURLOptimizer ?? .shared
        self.aiFingerprintTuning = aiFingerprintTuning ?? .shared
        self.aiSessionHealth = aiSessionHealth ?? .shared
        self.aiCredentialPriority = aiCredentialPriority ?? .shared
        self.aiAntiDetection = aiAntiDetection ?? .shared
        self.customTools = customTools ?? .shared
        self.aiInteractionGraph = aiInteractionGraph ?? .shared
        self.activityMonitor = activityMonitor ?? .shared
        self.liveSpeed = liveSpeed ?? .shared
        self.strictDetection = strictDetection ?? .shared
        self.crimsonOrchestrator = crimsonOrchestrator ?? .shared
        self.janitor = janitor ?? .shared
    }
    ...
    func runLoginTest(_ attempt: LoginAttempt, targetURL: URL, timeout: TimeInterval = 180) async -> LoginOutcome {
        ...
        logger.log("Config: timeout=\(Int(timeout))s stealth=\(stealthEnabled) activeSessions=\(activeSessions)/\(effectiveMaxConcurrency)", category: .login, level: .debug, sessionId: sessionId, metadata: ["url": targetURL.absoluteString, "username": attempt.credential.username])
        ...
        let outcome: LoginOutcome = await withTaskGroup(of: LoginOutcome.self) { group in
            ...
            group.addTask {
                let threshold = await MainActor.run { SessionActivityMonitor.idleThresholdSeconds }
                try? await Task.sleep(for: .seconds(threshold))
                while !Task.isCancelled {
                    let (idle, hasAny) = await MainActor.run {
                        (self.activityMonitor.isIdle(sessionId: sessionId),
                         self.activityMonitor.hasActivity(sessionId: sessionId))
                    }
                    if idle && !hasAny {
                        await MainActor.run {
                            self.logger.log("IdleWatchdog: \(sessionId) has ZERO network activity — killing", category: .webView, level: .error, sessionId: sessionId)
                        }
                        return .timeout
                    }
                    if idle && hasAny {
                        let secondsIdle = await MainActor.run { self.activityMonitor.secondsSinceLastActivity(sessionId: sessionId) }
                        let thresh = await MainActor.run { SessionActivityMonitor.idleThresholdSeconds }
                        if secondsIdle >= thresh {
                            await MainActor.run {
                                self.logger.log("IdleWatchdog: \(sessionId) went idle for \(Int(secondsIdle))s — killing", category: .webView, level: .error, sessionId: sessionId)
                            }
                            return .timeout
                        }
                    }
        ...
        for (pwIdx, password) in passwords.prefix(3).enumerated() {
            ...
            await withTaskGroup(of: (String, LoginOutcome).self) { group in
                for email in active {
                    ...
                    group.addTask {
                        let attempt = await MainActor.run { LoginAttempt(credential: LoginCredential(username: capturedEmail, password: password), sessionIndex: 0) }
        ...
        if verification.found < 2 {
            ...
            if verification.found == 0 {
                ...
                if retryVerification.found == 0 {
                    ...
                    await captureDebugScreenshot(session: session, attempt: attempt, step: "no_fields", note: "No login fields found", autoResult: .unknown)
                    return .connectionFailure
                }
            }
        } else {
    ...
    private func phasePatternCycleLoop(
        ...
        let enabledSet = Set(automationSettings.enabledPatterns)
        let priorityPatterns: [LoginFormPattern] = automationSettings.patternPriorityOrder
        ...
        for cycle in 1...maxSubmitCycles {
            ...
            if cycle == 1 {
                ...
                if automationSettings.patternLearningEnabled {
                    let learnedBest = humanEngine.selectBestPattern(for: targetURLString)
                    if enabledSet.contains(learnedBest.rawValue) {
                        selectedPattern = learnedBest
                        attempt.logs.append(PPSRLogEntry(message: "PatternML selected '\(learnedBest.rawValue)' as best for this site", level: .info))
                    } else {
                        selectedPattern = priorityPatterns.first ?? .visionMLCoordinate
                        attempt.logs.append(PPSRLogEntry(message: "PatternML suggested '\(learnedBest.rawValue)' but it's disabled — using priority first: \(selectedPattern.rawValue)", level: .info))
                    }
                } else {
                    selectedPattern = priorityPatterns.first ?? .visionMLCoordinate
                    attempt.logs.append(PPSRLogEntry(message: "Pattern learning disabled — using priority first: \(selectedPattern.rawValue)", level: .info))
                }
            } else {
            ...
            if cycle > 1 {
                ...
                if buttonReadyResult.ready {
                    ...
                    if !fallbackReady {
                        ...
                        return (.noAcc, lastEvaluation, maxSubmitCycles)
                    }
                }
            ...
            if contaminated {
                ...
                _ = try? await session.webView?.evaluateJavaScript(JSInteractionBuilder.humanTapJS(selector: emailSel))
                try? await Task.sleep(for: .milliseconds(automationSettings.preTypingDelayMs))
                _ = try? await session.webView?.evaluateJavaScript(JSInteractionBuilder.nativeSetterFillJS(selector: emailSel, value: attempt.credential.username))

                try? await Task.sleep(for: .milliseconds(automationSettings.interFieldDelayMs))
                _ = try? await session.webView?.evaluateJavaScript(JSInteractionBuilder.blurFieldJS(selector: emailSel))
                try? await Task.sleep(for: .milliseconds(50))

                _ = try? await session.webView?.evaluateJavaScript(JSInteractionBuilder.humanTapJS(selector: passSel))
                try? await Task.sleep(for: .milliseconds(automationSettings.preTypingDelayMs))
                _ = try? await session.webView?.evaluateJavaScript(JSInteractionBuilder.nativeSetterFillJS(selector: passSel, value: attempt.credential.password))
            ...
            logger.log("Rapid poll complete: success=\(pollResult.successTextFound) redirect=\(pollResult.redirectedToHomepage) nav=\(pollResult.navigationDetected) banner=\(pollResult.errorBannerDetected) sms=\(pollResult.smsNotificationDetected)", category: .automation, level: .debug, sessionId: sessionId, durationMs: pollMs)
            ...
            let successTextFound = pollResult.successTextFound
            let successContext: String? = pollResult.successTextFound ? String(pollResult.finalPageContent.prefix(200)) : nil

            attempt.logs.append(PPSRLogEntry(
                message: "Success text rapid poll: \(successTextFound ? "FOUND — \(successContext ?? "")" : "NOT FOUND")",
                level: successTextFound ? .success : .info
            ))
            attempt.logs.append(PPSRLogEntry(
            ...
            if pollResult.errorBannerDetected {
                attempt.logs.append(PPSRLogEntry(
                    message: "Error banner detected via rapid poll — classified as No Account (Crimson Sweep: DOM banner detection replaced by thickreddetect)",
                    level: .warning
                ))
                await captureDebugScreenshot(session: session, attempt: attempt, step: "error_banner_noAcc", note: "Error banner detected — classified as No Account")
                attempt.status = .failed
                attempt.errorMessage = "Error banner detected — No Account"
                attempt.completedAt = Date()
                return (.noAcc, lastEvaluation, maxSubmitCycles)
            }
            ...
            await captureAlwaysScreenshot(session: session, attempt: attempt, cycle: cycle, maxCycles: maxSubmitCycles, successTextFound: successTextFound, redirected: pollResult.redirectedToHomepage, evaluationReason: evaluation.reason, currentURL: currentURL, autoResult: autoResult)
    ...
    private func evaluateLoginResponse(
        ...
        return EvaluationResult(
            outcome: .noAcc,
            score: 10,
            reason: "No definitive keyword detected — classified as No Account — content: \"\(snippet)\"",
            signals: ["P4: no 'incorrect', no overrides — default No Account"]
        )
    }
    ...
    private func captureAlwaysScreenshot(session: LoginSiteWebSession, attempt: LoginAttempt, cycle: Int, maxCycles: Int, successTextFound: Bool, redirected: Bool, evaluationReason: String, currentURL: String, autoResult: PPSRDebugScreenshot.AutoDetectedResult) async {
        ...
        let screenshot = PPSRDebugScreenshot(
            ...
            image: compressed,
            note: "Cycle \(cycle)/\(maxCycles) | Success!: \(successTextFound ? "YES" : "NO") | Redirect: \(redirected ? "YES" : "NO") | \(evaluationReason) | URL: \(currentURL)\(noteExtra)",
    ...
    private func captureTerminalScreenshot(session: LoginSiteWebSession, attempt: LoginAttempt, step: String, note: String, autoResult: PPSRDebugScreenshot.AutoDetectedResult, terminalType: TerminalError) async {
        ...
        if terminalType == .errorBanner {
            terminalImage = await session.captureScreenshot()
        } else {
    ...
    private func captureTerminalMessageCrop(session: LoginSiteWebSession, terminalType: TerminalError) async -> UIImage? {
        ...
        case .accountDisabled:
            keywords = ["has been disabled", "account has been disabled", "account has been suspended", "permanently banned", "account is closed", "self-excluded", "account has been blocked"]
        case .errorBanner:
