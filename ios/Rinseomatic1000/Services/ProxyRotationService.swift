...
class ProxyRotationService {
    ...
    private func trimConfigsToOnePerProfile() {
        let migrationKey = "trim_configs_to_one_per_profile_v1"
        guard !UserDefaults.standard.bool(forKey: migrationKey) else { return }

        for profile in NordKeyProfile.allCases {
            for target: ProxyTarget in [.joe, .ignition, .ppsr] {
                let vpnConfigs = loadPersistedVPNConfigs(for: target, profile: profile)
                if vpnConfigs.count > 1 {
                    let kept = vpnConfigs.randomElement().map { [$0] } ?? []
                    persistVPNConfigs(kept, for: target, profile: profile)
                }

                let wgConfigs = loadPersistedWGConfigs(for: target, profile: profile)
                if wgConfigs.count > 1 {
                    let kept = wgConfigs.randomElement().map { [$0] } ?? []
                    persistWGConfigs(kept, for: target, profile: profile)
                }
            }
            logger.log("ProxyRotation: trimmed \(profile.rawValue) to 1 VPN + 1 WG config", category: .persistence, level: .info)
        }

        UserDefaults.standard.set(true, forKey: migrationKey)
        logger.log("ProxyRotation: config trim migration complete — kept max 4 configs total", category: .persistence, level: .success)
    }
    ...
    func connectionMode(for target: ProxyTarget) -> ConnectionMode {
        switch target {
        case .joe: return joeConnectionMode
        case .ignition: return ignitionConnectionMode
        case .ppsr: return ppsrConnectionMode
        default: return joeConnectionMode
        }
    }

    func proxies(for target: ProxyTarget) -> [ProxyConfig] {
        switch target {
        case .joe: return savedProxies
        case .ignition: return ignitionProxies
        case .ppsr: return ppsrProxies
        default: return []
        }
    }
    ...
    func bulkImportSOCKS5(_ text: String, for target: ProxyTarget) -> ImportReport {
        ...
        default: return bulkImportSOCKS5(text, forIgnition: false)
        }
    }
    ...
    func vpnConfigs(for target: ProxyTarget) -> [OpenVPNConfig] {
        switch target {
        case .joe: return joeVPNConfigs
        case .ignition: return ignitionVPNConfigs
        case .ppsr: return ppsrVPNConfigs
        default: return []
        }
    }
    ...
    func wgConfigs(for target: ProxyTarget) -> [WireGuardConfig] {
        switch target {
        case .joe: return joeWGConfigs
        case .ignition: return ignitionWGConfigs
        case .ppsr: return ppsrWGConfigs
        default: return []
        }
    }
