...
class DNSPoolService {
    ...
    private static let removedServerEndpoints: Set<String> = [
        "1.1.1.1",
        "dns.google",
        "dns.quad9.net",
        "dns.nextdns.io",
    ...
    var currentProvider: DoHProvider {
        ...
        guard !dohServers.isEmpty else { return DoHProvider(name: "Seby AU", url: "https://doh.seby.io/dns-query") }
    ...
    private func stripRemovedServers() {
        let before = managedServers.count
        managedServers.removeAll { server in
            server.protocolType == .dot && Self.removedServerEndpoints.contains(server.endpoint)
        }
        if managedServers.count != before {
