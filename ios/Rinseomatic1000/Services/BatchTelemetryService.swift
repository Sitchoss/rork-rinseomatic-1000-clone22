...
actor BatchTelemetryService {
    ...
    private nonisolated var logger: DebugLogger { DebugLogger.shared }
    ...
    func endBatch() {
        ...
        Task { @MainActor in
            DebugLogger.shared.log("Telemetry: batch \(batch.batchId) complete — \(batch.successCount)/\(batch.processedItems) success (\(Int(batch.successRate * 100))%), \(batch.durationSeconds)s, \(String(format: "%.1f", batch.throughputPerMinute))/min, \(batch.ipRotations) rotations, \(batch.autoPauseTriggers) pauses", category: .automation, level: .info)
        }
    ...
    func clearHistory() {
        ...
        persistRecords()
        Task { @MainActor in
            DebugLogger.shared.log("Telemetry: batch history cleared", category: .automation, level: .info)
        }
    }
    ...
    private func loadRecords() {
        ...
        let count = batchRecords.count
        Task { @MainActor in
            DebugLogger.shared.log("Telemetry: loaded \(count) batch records", category: .automation, level: .info)
        }
    }
}
