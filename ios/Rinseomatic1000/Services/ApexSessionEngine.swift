...
class LoginSiteWebSession: NSObject {
    ...
    private(set) var poolSessionId: UUID = UUID()
    private var trackerSessionId: String {
        poolSessionId.uuidString.prefix(8).lowercased() + "-" + (targetURL.host ?? "unknown")
    }
    ...
    func setUp(wipeAll: Bool = false) async {
        ...
        contentController.addUserScript(CookieBannerBlockerScript.userScript)
        ...
        if let wv = webView {
            WebViewPool.shared.mount(wv, for: poolSessionId)
        }
    ...
    func tearDown(wipeAll: Bool = false) {
        ...
        if webView != nil {
            ...
            WebViewPool.shared.unmount(id: poolSessionId)
        }
        webView = nil
        isPageLoaded = false
    ...
    private func executeCookieDismissPass_DISABLED() async -> String {
        let js = """
        (function() {
        ...
        """
        return await executeJS(js) ?? "NONE_FOUND"
    }
    ...
    func verifyLoginFieldsExist() async -> (found: Int, missing: [String]) {
        ...
        (function() {
            var missing = [];
            var found = 0;
            var emailSelectors = ['#username','#email','input[type="email"]','input[name="email"]','input[name="username"]','#loginEmail','input[autocomplete="email"]','input[autocomplete="username"]'];
            var passSelectors = ['#password','#login-password','input[type="password"]','input[name="password"]','#loginPassword','input[autocomplete="current-password"]'];
    ...
    func autoCalibrate() async -> LoginCalibrationService.URLCalibration? {
        let js = """
        (function() {
            var emailSelectors = ['#username','#email','input[type="email"]','input[name="email"]','input[name="username"]','#loginEmail','input[autocomplete="email"]','input[autocomplete="username"]','input[type="text"]:first-of-type'];
            var passSelectors = ['#password','#login-password','input[type="password"]','input[name="password"]','#loginPassword','input[autocomplete="current-password"]'];
            var btnSelectors = ['#loginSubmit','#login-submit','button[type="submit"]','input[type="submit"]','#loginButton','button.login-button'];
    ...
    func rapidResponsePoll(timeout: TimeInterval, originalURL: String) async -> (successTextFound: Bool, redirectedToHomepage: Bool, navigationDetected: Bool, errorBannerDetected: Bool, smsNotificationDetected: Bool, finalPageContent: String, finalURL: String) {
        ...
        (function() {
            ...
            var successTerms = ['my account','logged in','lobby','your balance','my wallet','logout','log out','recommended for you','last played'];
        ...
        if let raw = await executeJS(pollJS),
           ...
           let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            ...
            successTextFound = json["success"] as? Bool ?? false
    ...
    func checkLoginButtonReadiness() async -> Bool {
        let js = """
        (function() {
            var selectors = ['#loginSubmit','#login-submit','button[type="submit"]','input[type="submit"]','#loginButton','button.login-button'];
            var loginTerms = ['log in','login','sign in','signin','submit'];
            var btn = null;
            for (var i = 0; i < selectors.length; i++) {
                var el = document.querySelector(selectors[i]);
    ...
    func clearEmailFieldOnly() async {
        let js = """
        (function() {
            var selectors = ['#username', '#email', 'input[type="email"]', 'input#email', 'input[name="email"]', 'input[name="username"]', 'input[type="text"]:first-of-type'];
            for (var i = 0; i < selectors.length; i++) {
                var el = document.querySelector(selectors[i]);
                if (el) {
                    el.focus();
...
class LoginWebSession: NSObject, ScreenshotCapableSession {
    ...
    private static let blockResourcesRuleListID = "Rinseomatic1000BlockHeavyResources"
    ...
    func setUp() {
        ...
        contentController.addUserScript(ApexConstants.keyboardSuppressionScript)
        CookieBannerBlockerScript.addIfEnabled(to: contentController)
        config.userContentController = contentController
...
class DeadSessionDetector {
    ...
    func isSessionAlive(_ webView: WKWebView?, sessionId: String = "") async -> Bool {
        ...
        let alive = await heartbeatTask.value
        timeoutTask.cancel()
