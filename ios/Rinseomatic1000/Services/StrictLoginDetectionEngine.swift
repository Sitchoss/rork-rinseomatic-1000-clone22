...
class StrictLoginDetectionEngine {
    ...
    func evaluateImmediateOverrides(
        ...
        let trimmedContent = pageContent.trimmingCharacters(in: .whitespacesAndNewlines)

        // Check for about:blank or truly empty content (after strong signals)
        if contentLower.contains("about:blank") || trimmedContent.isEmpty {
            logger.log("StrictDetection P1: CONNECTION_FAILURE — about:blank or empty content", category: .evaluation, level: .warning, sessionId: sessionId)
            return .connectionFailure
        }

        // Check for SMS detection if enabled
        if automationSettings.smsDetectionEnabled {
            for keyword in automationSettings.smsNotificationKeywords {
                if contentLower.contains(keyword.lowercased()) {
                    logger.log("StrictDetection P1: SMS_DETECTED — '\(keyword)' in DOM", category: .evaluation, level: .critical, sessionId: sessionId)
                    return .smsDetected
                }
            }
        }
        ...
        if let img = screenshot {
            ...
            if !domHasSuccess && !ocrHasSuccess {
                // Check SMS via OCR if enabled
                if automationSettings.smsDetectionEnabled {
                    for keyword in automationSettings.smsNotificationKeywords {
                        if ocrLower.contains(keyword.lowercased()) {
                            logger.log("StrictDetection P1: SMS_DETECTED — '\(keyword)' via OCR", category: .evaluation, level: .critical, sessionId: sessionId)
                            return .smsDetected
                        }
                    }
                }
    ...
    func evaluatePostSubmit(
        ...
        let p1Override = await evaluateImmediateOverrides(
            ...
            executeJS: { script in
                await session.executeJS(script)
            }
        )
        ...
        try? await Task.sleep(for: .milliseconds(automationSettings.pageStabilizationDelayMs))

        let settledContent = (await session.getPageContent() ?? "").lowercased()

        if settledContent.contains("incorrect") {
            logger.log("StrictDetection P3: 'incorrect' found in DOM after \(automationSettings.pageStabilizationDelayMs)ms wait", category: .evaluation, level: .info, sessionId: sessionId)
            return DetectionResult(
                outcome: .noAcc,
                phase: "P3_DOM",
        ...
        logger.log("StrictDetection P4-Step9: No 'incorrect' detected after full first attempt + retry — classifying as No Account", category: .evaluation, level: .warning, sessionId: sessionId)
        return DetectionResult(
            outcome: .noAcc,
            phase: "P4_final",
            reason: "Neither DOM nor OCR detected 'incorrect' after complete first attempt + full retry — classified as No Account",
            incorrectDetectedViaDOM: false,
            incorrectDetectedViaOCR: false,
            buttonCycleCompleted: buttonCycleCompleted,
            retryPerformed: true,
    ...
    func evaluateStrict(
        ...
        return DetectionResult(
            outcome: .noAcc,
            phase: "P4_final",
            reason: "No 'incorrect' detected via DOM or OCR — classified as No Account",
            incorrectDetectedViaDOM: false,
            incorrectDetectedViaOCR: false,
            buttonCycleCompleted: true,
            retryPerformed: false,
    ...
    func runStandardLoginDetection(
        ...
        if let fingerprint = preClickFingerprint {
            ...
            try? await Task.sleep(for: .milliseconds(automationSettings.pageStabilizationDelayMs))
            buttonCycleOk = true
            onLog?("StrictDetection P3: no fingerprint captured — used stabilization delay", .warning)
        }

        let firstAttemptResult = await evaluatePostSubmitDOM(session: session, sessionId: sessionId, label: "P3", settleMs: automationSettings.pageStabilizationDelayMs)
        ...
        let retryDOMResult = await evaluatePostSubmitDOM(session: session, sessionId: sessionId, label: "P4-retry", settleMs: automationSettings.pageStabilizationDelayMs)
        ...
        onLog?("StrictDetection P4-Step9: No Account — neither DOM nor OCR found 'incorrect' after full retry", .error)
        return DetectionResult(
            outcome: .noAcc,
            phase: "P4_final",
            reason: "Complete first attempt + full retry: no 'incorrect' detected anywhere — classified as No Account",
            incorrectDetectedViaDOM: false,
            incorrectDetectedViaOCR: false,
            buttonCycleCompleted: true,
            retryPerformed: true,
    ...
    private func evaluatePostSubmitDOM(
        ...
        let resolvedMs = settleMs ?? AutomationSettingsPersistence.shared.load().pageStabilizationDelayMs
        try? await Task.sleep(for: .milliseconds(resolvedMs))
