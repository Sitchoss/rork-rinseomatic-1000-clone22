...
public enum HyperFlowTaskFactory {
    ...
    public static func loginDualSitePair(
        ...
        PairedTask(
            ...
            primaryURL: joeURL ?? URL(string: "https://www.joefortunepokies.eu/login")!,
            secondaryURL: ignitionURL ?? URL(string: "https://www.ignitioncasino.ooo/?overlay=login")!,
...
nonisolated enum LoginTargetSite: String, CaseIterable, Sendable {
    ...
    var url: URL {
        switch self {
        case .joefortune: URL(string: "https://www.joefortunepokies.eu/login")!
        case .ignition: URL(string: "https://www.ignitioncasino.ooo/?overlay=login")!
        }
    }

    var host: String {
        switch self {
        case .joefortune: "www.joefortunepokies.eu"
        case .ignition: "www.ignitioncasino.ooo"
        }
    }
    ...
    var passwordSelector: String {
        switch self {
        case .joefortune: "#password"
        case .ignition: "#login-password"
        }
    }

    var submitSelector: String {
        switch self {
        case .joefortune: "#loginSubmit"
        case .ignition: "#login-submit"
        }
    }

    var siteTarget: SiteTarget {
        switch self {
        case .joefortune: .joefortune
