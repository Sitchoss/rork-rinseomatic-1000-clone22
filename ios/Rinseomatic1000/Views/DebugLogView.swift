...
struct DebugLogView: View {
        ...
        .toolbar {
            ToolbarItemGroup(placement: .topBarTrailing) {
                ...
                Menu {
                    ...
                    Divider()
                    Button { exportAutomationDiagnostics() } label: {
                        Label("Export Automation Diagnostics", systemImage: "stethoscope.circle.fill")
                    }
                    Button { exportAutomationDiagnosticsAsFile() } label: {
                        Label("Share Automation Diagnostics File", systemImage: "square.and.arrow.up")
                    }
                    Divider()
    ...
    private var bottomBar: some View {
        ...
        return HStack(spacing: 12) {
            ...
            if logger.automationOnlyMode {
                Text("\u{1F507} Auto-Only")
    ...
    private func exportAutomationDiagnostics() {
        exportText = AutomationDiagnosticExporter.shared.exportAutomationDiagnostics()
        showExportSheet = true
    }

    private func exportAutomationDiagnosticsAsFile() {
        if let url = AutomationDiagnosticExporter.shared.exportToFile() {
            shareFileURL = url
        }
    }
