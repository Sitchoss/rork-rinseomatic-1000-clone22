    ...
    private var textboxMappingsSection: some View {
        Section {
            if flow.textboxMappings.isEmpty {
                ...
                ForEach(Array(flow.textboxMappings.enumerated()), id: \.element.id) { index, mapping in
                    VStack(alignment: .leading, spacing: 6) {
                        Text(mapping.selector.isEmpty ? mapping.label : mapping.selector)
                        ...
                        HStack(spacing: 8) {
                            ...
                            Picker("Token", selection: Binding(
                                get: { flow.textboxMappings[index].assignedToken },
                                set: { flow.textboxMappings[index].assignedToken = $0 }
                            )) {
                                Text("None").tag(FlowPlaceholderToken?.none)
                                ForEach(FlowPlaceholderToken.allCases, id: \.self) { token in
                                    Text(token.displayLabel).tag(FlowPlaceholderToken?.some(token))
                                }
                            }
                            .pickerStyle(.menu)
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let selector = action.targetSelector, !selector.isEmpty {
                    Text(selector)
...
struct TextboxMappingEditorView: View {
    ...
    @State private var newToken: FlowPlaceholderToken?

    var body: some View {
        Form {
            Section("Add Mapping") {
                TextField("CSS Selector (e.g. #email)", text: $newSelector)
                    .font(.system(.body, design: .monospaced))
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                TextField("Label (optional)", text: $newLabel)

                Picker("Token", selection: $newToken) {
                    Text("None").tag(FlowPlaceholderToken?.none)
                    ForEach(FlowPlaceholderToken.allCases, id: \.self) { token in
                        Text(token.displayLabel).tag(FlowPlaceholderToken?.some(token))
                    }
                ...
                Button {
                    guard !newSelector.isEmpty else { return }
                    let label = newLabel.isEmpty ? newSelector : newLabel
                    ...
                    ForEach(Array(mappings.enumerated()), id: \.element.id) { index, mapping in
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            HStack(spacing: 8) {
                                ...
                                Picker("Token", selection: Binding(
                                    get: { mappings[index].assignedToken },
                                    set: { mappings[index].assignedToken = $0 }
                                )) {
                                    Text("None").tag(FlowPlaceholderToken?.none)
                                    ForEach(FlowPlaceholderToken.allCases, id: \.self) { token in
                                        Text(token.displayLabel).tag(FlowPlaceholderToken?.some(token))
