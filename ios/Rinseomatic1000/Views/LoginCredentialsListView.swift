    ...
    private var sortFilterBar: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 6) {
                        LoginFilterChip(title: "All", isSelected: filterStatus == nil, accent: hubAccent) { withAnimation(.snappy) { filterStatus = nil } }
                        LoginFilterChip(title: "Working", isSelected: filterStatus == .working, accent: hubAccent) { withAnimation(.snappy) { filterStatus = .working } }
                        LoginFilterChip(title: "Untested", isSelected: filterStatus == .untested, accent: hubAccent) { withAnimation(.snappy) { filterStatus = .untested } }
                        LoginFilterChip(title: "No Acc", isSelected: filterStatus == .noAcc, accent: hubAccent) { withAnimation(.snappy) { filterStatus = .noAcc } }
                        LoginFilterChip(title: "Perm Dis", isSelected: filterStatus == .permDisabled, accent: hubAccent) { withAnimation(.snappy) { filterStatus = .permDisabled } }
                        LoginFilterChip(title: "Temp Dis", isSelected: filterStatus == .tempDisabled, accent: hubAccent) { withAnimation(.snappy) { filterStatus = .tempDisabled } }
    ...
    var body: some View {
        Button(action: action) {
            Text(title).font(.caption)
                ...
                .background(isSelected ? accent : Color(.tertiarySystemFill))
