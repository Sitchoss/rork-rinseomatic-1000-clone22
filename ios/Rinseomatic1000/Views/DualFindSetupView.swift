...
struct DualFindSetupView: View {
            ...
            HStack(spacing: 10) {
                ...
                Button {
                    if let clipboard = UIPasteboard.general.string, !clipboard.isEmpty {
                        vm.emailInputText = clipboard
                        vm.clearPasteSpecialStatus()
                    }
                } label: {
    ...
    private var passwordSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 10) {
                ...
                Button {
                    if let clipboard = UIPasteboard.general.string, !clipboard.isEmpty {
                        vm.passwordInputText = clipboard
                        vm.clearPasteSpecialStatus()
                    }
                } label: {
                    Label("Paste", systemImage: "doc.on.clipboard")
    ...
    private var pasteSpecialSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("Paste Special")
                        .font(.system(size: 14, weight: .bold))
                    Text("Pulls emails above PASSWORDS (case-sensitive) and cleans passwords below it.")
                        .font(.system(size: 11, weight: .medium))
                        .foregroundStyle(.secondary)
                }
            ...
            HStack(spacing: 10) {
                Button {
                    vm.importPasteSpecialFromClipboard()
                } label: {
                    Label("Paste Special", systemImage: "doc.on.clipboard.fill")
                ...
                .buttonStyle(.bordered)
                .tint(.secondary)
                .disabled(vm.pasteSpecialStatusMessage.isEmpty)
            }

            HStack(spacing: 8) {
                Label(">7 chars", systemImage: "line.3.horizontal.decrease.circle")
                Label("No numbers-only", systemImage: "number")
                Label("No letters-only", systemImage: "textformat.abc")
            }
            .font(.system(size: 10, weight: .semibold))
            .foregroundStyle(.secondary)

            if !vm.pasteSpecialStatusMessage.isEmpty {
                HStack(spacing: 8) {
                    Image(systemName: vm.pasteSpecialStatusIsError ? "exclamationmark.triangle.fill" : "checkmark.circle.fill")
                        .foregroundStyle(vm.pasteSpecialStatusIsError ? .orange : .green)
                    Text(vm.pasteSpecialStatusMessage)
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundStyle(vm.pasteSpecialStatusIsError ? .orange : .green)
                    Spacer()
                }
                .padding(10)
                .background((vm.pasteSpecialStatusIsError ? Color.orange : Color.green).opacity(0.10))
                .clipShape(.rect(cornerRadius: 10))
            }
        }
        .padding(14)
        ...
        Button {
            ...
            VStack(spacing: 4) {
                ...
                if vm.parsedPasswordSetCount > 1 {
                    ...
                    Text("Enter emails and passwords above")
                        .font(.system(size: 11, weight: .medium))
                        .opacity(0.6)
                }
            }
