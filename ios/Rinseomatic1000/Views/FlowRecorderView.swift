...
struct FlowRecorderView: View {
    ...
    init(viewModel: FlowRecorderViewModel? = nil, initialURL: String = "https://joefortunepokies.eu/login") {
        if let viewModel {
            viewModel.targetURL = initialURL
            _vm = State(initialValue: viewModel)
        } else {
            _vm = State(initialValue: FlowRecorderViewModel(targetURL: initialURL))
        }
    }
        ...
        NavigationStack {
            Form {
                ...
                if !vm.detectedTextboxes.isEmpty {
                    Section("Detected Text Fields — Assign Tokens") {
                        ForEach(vm.detectedTextboxes, id: \.self) { label in
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                HStack(spacing: 8) {
                                    ...
                                    let currentToken: FlowPlaceholderToken? = {
                                        if let override = vm.pendingTokenOverrides[label] {
                                            return override
                                        }
                                        let lastInput = vm.currentActions.last(where: { $0.textboxLabel == label && $0.type == .input })
                                        return FlowPlaceholderToken.detect(
                                            fromLabel: label,
                                            targetType: lastInput?.targetType,
                                    ...
                                    Picker("Token", selection: Binding(
                                        get: { currentToken },
                                        set: { vm.pendingTokenOverrides[label] = $0 }
                                    )) {
