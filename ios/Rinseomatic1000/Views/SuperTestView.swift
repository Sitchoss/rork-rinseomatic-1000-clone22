                ...
                Button {
                    ...
                    VStack(spacing: 4) {
                        HStack(spacing: 8) {
                            ...
                            Text("Run Super Test")
                                .font(.subheadline.bold())
                            if service.selectedConnectionTypes.count < SuperTestConnectionType.allCases.count && !service.selectedConnectionTypes.isEmpty {
                                Text("(\(service.selectedConnectionTypes.count)/\(SuperTestConnectionType.allCases.count))")
                                    .font(.caption2)
                                    .opacity(0.8)
                            }
                        }
                        if service.selectedConnectionTypes.isEmpty {
                            Text("Select at least one test phase above")
