...
struct DualFindRunningView: View {
    ...
    private var telemetry: AITelemetryService { AITelemetryService.shared }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 14) {
                    ...
                    if let summary = telemetry.currentBatchSummary {
                        AIBatchSummaryCard(summary: summary)
                    }
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Button { showDeveloperSettings = true } label: {
        ...
        .sensoryFeedback(.warning, trigger: vm.interventionNoAccountCount)
    }
    ...
    private func sessionCard(_ session: DualFindSessionInfo) -> some View {
        ...
        let stealthKey = "\(isJoe ? "JOE" : "IGN")-\(session.index)"
        let stealth = telemetry.stealthScore(for: stealthKey)
        ...
        return VStack(alignment: .leading, spacing: 4) {
            ...
            if let stealth, stealth.tier == .critical {
                AILowStealthBanner(score: stealth)
            }
