...
struct SavedFlowsView: View {
    ...
    @State private var assignmentService = FlowScriptAssignmentService.shared
    private let automationSettings: AutomationSettings = AutomationSettingsPersistence.shared.load()
                ...
                VStack(alignment: .trailing, spacing: 4) {
                    ...
                    HStack(spacing: 6) {
                        Text("Est:")
                            .font(.system(size: 9, weight: .semibold, design: .monospaced))
                            .foregroundStyle(.secondary)
                        Text(FlowTimeEstimator.formattedEstimate(for: flow, settings: automationSettings))
                            .font(.system(size: 12, weight: .bold, design: .monospaced))
    ...
    private func groupActions(_ actions: [RecordedAction]) -> [ActionGroup] {
        ...
        for action in actions {
            ...
            case .mouseUp:
                ...
                desc = "mouseup \(hold)"
            case .keyDown:
                let sel = action.targetSelector ?? action.textboxLabel ?? ""
                desc = "keydown '\(action.key ?? "?")' \(sel)"
            case .keyUp:
                desc = "keyup '\(action.key ?? "?")'"
            case .input:
                let selector = action.targetSelector ?? action.textboxLabel ?? "field"
                desc = "input → \(selector)"
            case .focus:
