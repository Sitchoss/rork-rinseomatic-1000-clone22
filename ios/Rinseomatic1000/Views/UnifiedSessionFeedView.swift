                ...
                ToolbarItem(placement: .topBarLeading) {
                    Menu {
                        Button { showDeveloperSettings = true } label: {
                            Label("Settings Hub", systemImage: "gearshape.2.fill")
                        }
                        Divider()
            ...
            Button { vm.startBatch() } label: {
                VStack(spacing: 4) {
                    ...
                    if vm.pendingSessions.isEmpty {
                        Text("Import credentials to begin")
                            .font(.system(.caption2, design: .monospaced))
        ...
        NavigationStack {
            List {
                ...
                Section("Export by Site Result") {
                    ForEach([SiteResult.success, .permDisabled, .tempDisabled, .noAccount], id: \.rawValue) { result in
