...
struct LoginMoreMenuView: View {
    ...
    private var advancedSettingsSection: some View {
        Section {
            NavigationLink {
                ...
                moreRow(icon: "gearshape.fill", title: "Advanced Settings", subtitle: "Debug, diagnostics & data management", color: .secondary)
            }
        }
    }
    ...
    private var settingsAndTestingLinkSection: some View {
        Section {
            Button {
                UserDefaults.standard.set(ActiveAppMode.toolsAndTesting.rawValue, forKey: "activeAppMode")
            } label: {
                HStack(spacing: 12) {
                    ZStack {
                        Circle()
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Settings Hub").font(.subheadline.bold())
                        Text("Automation, Network, URLs, Testing, Advanced & more")
                            .font(.caption2).foregroundStyle(.secondary)
                    }
                    Spacer()
