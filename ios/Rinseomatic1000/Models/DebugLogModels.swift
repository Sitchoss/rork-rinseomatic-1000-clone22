...
nonisolated enum DebugLogCategory: String, CaseIterable, Sendable, Identifiable, Codable {
    ...
    case aiTelemetry = "AI Telemetry"

    var id: String { rawValue }

    var isAutomationRelevant: Bool {
        switch self {
        case .automation, .login, .ppsr, .superTest, .webView,
             .fingerprint, .stealth, .evaluation, .timing,
             .healing, .flowRecorder, .url, .aiTelemetry:
            true
        case .network, .proxy, .dns, .vpn, .persistence,
             .system, .screenshot:
            false
        }
    }

    static let automationOnlySet: Set<DebugLogCategory> = {
        Set(allCases.filter(\.isAutomationRelevant))
    }()
